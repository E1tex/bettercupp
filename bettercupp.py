...
def print_to_file(filename, unique_list_finished, ni_mode):
    f = open(filename, "w")
...
def generate_wordlist_from_profile(profile):
    ...
    profile["spechars"] = []
    if profile["spechars1"] == "y" or profile["spechars1"] == True:
        for spec1 in chars:
...
def get_parser():
    ...
    group.add_argument(
        ...
        "--noninteractive",
        action="store_true",
        help="Non Interactive mode for user password profiling"
    )
    parser.add_argument(
        "-fn",
        "--firstname",
        help="Target First Name for -ni(Non Interactive) mode"
    )
    parser.add_argument(
        "-ln",
        "--lastname",
        help="Target Last Name for -ni(Non Interactive) mode"
    )
    parser.add_argument(
        "-n",
        "--nickname",
        help="Target Nickname for -ni(Non Interactive) mode"
    )
    parser.add_argument(
        "-bd",
        "--birthdate",
        help="Target Birthdate for -ni(Non Interactive) mode"
    )
    parser.add_argument(
        "-pfn",
        "--partners_firstname",
        help="Partners First Name for -ni(Non Interactive) mode"
    )
    parser.add_argument(
        "-pn",
        "--partners_nickname",
        help="Partners Nickname:"
    )
    parser.add_argument(
        "-pbd",
        "--partners_birthdate",
        help="Partners birthdate (DDMMYYYY):"
    )
    parser.add_argument(
        "-cn",
        "--childs_name",
    ...
    parser.add_argument(
        "-cni",
        "--childs_nickname",
        help="Child's nickname"
    )
    parser.add_argument(
        "-cbd",
        "--childs_birthdate",
        help="Child's birthdate (DDMMYYYY)"
    )
    parser.add_argument(
        "-petn",
        "--pets_name",
    ...
    parser.add_argument(
        ...
        "--special_chars",
        action="store_true",
        help="Add special chars"
    )
    parser.add_argument(
        "-rn",
        "--add_random_numbers",
        action="store_true",
        help="Random numbers at the end of words"
    )
    parser.add_argument(
        "-lm",
        "--leet_mode",
